...
const SampleConfigMapping &GetSampleConfigurationMapping() {
    static SampleConfigMapping sampleConfigMapping = {
        {T_RH_V3,
          ...
          .sampleSlots =
              {
                  ...
                  {SignalType::CO2_PARTS_PER_MILLION,
                   {.signalType = SignalType::CO2_PARTS_PER_MILLION,
                    ...
                    .encodingFunction = &encodeSimple,
                    .decodingFunction = &decodeSimple}}
                  // 2 Bytes reserved (ALT), not used, but bytes must be sent
                  // for proper decoding of samples
              }}},
        {T_RH_CO2_PM25,
        ...
        {AV_T,
         {.dataType = DataType::AV_T,
          ...
          .sensirionAdvertisementSampleType = 0,
          .sampleSlots =
              {{SignalType::VELOCITY_METERS_PER_SECOND,
...
SampleConfig GetSampleConfiguration(const DataType dataType) {
...
// Converter functions
uint16_t encodeSimple(const float value) {
    return static_cast<uint16_t>(value + 0.5f);
}

uint16_t encodeTemperatureV1(const float value) {
    return static_cast<uint16_t>((((value + 45) / 175) * 65535) + 0.5f);
}

uint16_t encodeHumidityV1(const float value) {
    return static_cast<uint16_t>(((value / 100) * 65535) + 0.5f);
}

uint16_t encodeHumidityV2(const float value) {
    return static_cast<uint16_t>((((value + 6.0) * 65535) / 125.0) + 0.5f);
}

uint16_t encodePM2p5V1(const float value) {
    return static_cast<uint16_t>(((value / 1000) * 65535) + 0.5f);
}

uint16_t encodePMV2(const float value) {
    return static_cast<uint16_t>((value * 10) + 0.5f);
}

uint16_t encodeHCHOV1(const float value) {
    return static_cast<uint16_t>((value * 5) + 0.5f);
}

uint16_t encodeVelocityV1(const float value) {
    return static_cast<uint16_t>((value / 1024) * 65535 + 0.5f);
}

uint16_t encodeH2ConcentrationV1(const float value) {
    return static_cast<uint16_t>((value * 100.0f) + 0.5f);
}
/* Define new encoding functions here */

float decodeSimple(const uint16_t rawValue) {
    return static_cast<float>(rawValue);
}

float decodeTemperatureV1(const uint16_t rawValue) {
    return -45.0f + ((175.0f * static_cast<float>(rawValue)) / 65535);
}

float decodeHumidityV1(const uint16_t rawValue) {
    return (100.0f * static_cast<float>(rawValue)) / 65535;
}

float decodeHumidityV2(const uint16_t rawValue) {
    return (125.0f * static_cast<float>(rawValue)) / 65535 - 6.0f;
}

float decodePM2p5V1(const uint16_t rawValue) {
    return (1000.0f * static_cast<float>(rawValue)) / 65535;
}

float decodePMV2(const uint16_t rawValue) {
    return static_cast<float>(rawValue) / 10.0f;
}

float decodeHCHOV1(const uint16_t rawValue) {
    return static_cast<float>(rawValue) / 5.0f;
}

float decodeVelocityV1(const uint16_t rawValue) {
    return static_cast<float>(rawValue * 1024.0 / 65535.0);
}

float decodeH2ConcentrationV1(const uint16_t rawValue) {
    return static_cast<float>(rawValue) / 100.0f;
}
/* Define new decoding functions here */

void emplaceRawValue(std::string &data, const uint8_t offset,
...
uint16_t getRawValue(const std::string &data, const uint8_t offset) {
    uint16_t rawValue = 0;
    if (offset + sizeof(uint16_t) <= data.size()) {
        rawValue =
...
DataType getDataTypeFromSampleType(const uint8_t sampleType) {
    // Look through the config selector for a matching entry.
    auto mapping = GetSampleConfigurationMapping();
    for (const auto &[dataType, sampleConfig] : mapping) {

        if (sampleConfig.sampleType == sampleType) {
            return dataType;
        }
    }
    return UNDEFINED;
...
void InitSampleConfigurationMapping() { GetSampleConfigurationMapping(); }
